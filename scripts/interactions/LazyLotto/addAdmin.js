...
const main = async () => {
	...
	if (args.length !== 2 || getArgFlag('h')) {
		console.log('Usage: addAdmin.js <contractId> <adminAccountId>');
		return;
	}
	const contractId = ContractId.fromString(args[0]);
	const newAdmin = AccountId.fromString(args[1]);

	// import ABI
	const abi = JSON.parse(fs.readFileSync(`./abi/${contractName}.json`));
	const iface = new ethers.Interface(abi);

	const proceed = readlineSync.keyInYNStrict(`Add ${newAdmin.toString()} as admin to contract ${contractId.toString()}?`);
	if (!proceed) {
	...
	const result = await contractExecuteFunction(
		...
		200_000,
		'addAdmin',
		[newAdmin.toSolidityAddress()]
	);

	if (result[0]?.status?.toString() != 'SUCCESS') {
		console.log('Error adding admin:', result);
		return;
	}
