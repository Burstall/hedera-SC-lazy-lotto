...
(async () => {
	...
	if (!poolId) {
		console.error('Usage: node claimAllPrizes.js <poolId>');
		process.exit(1);
	}

	const params = new ContractFunctionParameters().addUint256(poolId);
	const { contractExecuteFunction } = require('../../utils/solidityHelpers');
	const tx = await contractExecuteFunction(client, contractId, params, 0, 'claimAllPrizes', 200000);
	await tx.getReceipt(client);
	console.log('claimAllPrizes executed successfully.');
})();
