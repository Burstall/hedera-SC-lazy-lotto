...
const main = async () => {
	...
	if (args.length < 2 || getArgFlag('h')) {
		console.log('Usage: isAdmin.js 0.0.LTL USER_ADDRESS');
	...
	const isAdminCommand = ltlIface.encodeFunctionData('isAdmin', [userAddress]);
	const isAdminResponse = await readOnlyEVMFromMirrorNode(
		env, contractId, isAdminCommand, operatorId, false
	);
	const isAdmin = ltlIface.decodeFunctionResult('isAdmin', isAdminResponse)[0];
