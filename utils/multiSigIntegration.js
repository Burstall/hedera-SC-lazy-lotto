...
async function contractExecuteFunctionMultiSig(
	...
	try {
		...
		if (result.success && result.receipt) {
			...
			const transactionId = result.transactionId || receipt.transactionId?.toString();

			// In multi-sig mode, we can't call getRecord() because it requires signing
			// Instead, we fetch the contract result from the mirror node (free, no signing)
			let contractResults = null;

			if (transactionId) {
				try {
					// Get environment from process.env (set by admin scripts)
					const env = process.env.ENVIRONMENT || 'testnet';

					// Convert transaction ID to mirror node format
					const mirrorTxId = translateTransactionForWebCall(transactionId);

					console.log('\n📡 Fetching contract result from mirror node...');

					const mirrorResult = await getContractResultWithRetry(
						env,
						mirrorTxId,
						iface,
						{
					...
					if (mirrorResult.success && mirrorResult.call_result) {
						contractResults = mirrorResult.call_result;
					}
					else if (mirrorResult.error) {
						if (flagError) {
							console.log('Note: Could not decode contract result:', mirrorResult.error);
						}
					}
				}
				catch (e) {
					if (flagError) {
						console.log('Note: Error fetching result from mirror node:', e.message);
					}
				}
			}

			return [receipt, contractResults, receipt];
		}
		else {
			// Handle errors
			if (flagError) {
